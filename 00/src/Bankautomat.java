...
void main() {
    ...
    int currVal = Integer.parseInt(IO.readln("Bitte Startguthaben eingeben: "));
    ...
    while (!aktion.equals("STOP")) {
        switch (aktion) {
            case "GUTHABEN" -> IO.println("Ihr Guthaben beträgt " + currVal + " Euro. ");
            case "EINZAHLEN" -> {
            ...
            case "ABHEBEN" -> {
                ...
                if (val < 0) {
                    ...
                    IO.println("Guthaben nicht ausreichend!");
                } else {
                    currVal = currVal - val;
                    IO.println("Ihr Guthaben beträgt " + currVal + " EURO.");
                }
            ...
            case "ZINSESZINS" -> {
                ...
                for (int i = 0; i < n; i++) {
                    currVal = currVal + (int) Math.ceil(currVal * 0.04);
                    currVal = currVal - 2;
                    if (currVal < 0) {
                        currVal = 0;
            ...
            default -> IO.println("Invalide Eingabe");
        }
        aktion = IO.readln("Bitte geben Sie eine Operation (GUTHABEN, EINZAHLEN, ABHEBEN, ZINSESZINS, STOP) ein: ");
    }
    IO.println("Ihr Guthaben beträgt " + currVal + " EURO. Vielen Dank und bis zum nächsten Mal!");
}
